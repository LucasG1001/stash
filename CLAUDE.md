...
- **Filtros/ordenação da biblioteca — lógica com base em coleções (invariantes):**
  ...
  - **Filtro reduz a coleção**: os filtros de status são **multi-seleção** (arrays) e viram um
    `memberFilter` passado ao builder — ele **reduz os `members` aos que batem** e recalcula
    capa/`count`/`completedCount`; coleção sem match some. Sem filtro (array vazio): mostra tudo
    (inclusive `dropped`), escondendo só coleções 100% `dropped` (`.filter(some não-dropped)` na página).
    Reduzir a 1 item faz o grupo renderizar como card avulso. `filterGroupsBySearch` continua por título.
